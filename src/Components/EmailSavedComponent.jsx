...
function EmailSavedComponent(props){
    ...
    let dateandtime = date_moment.format("dd, MMM DD YYYY")
    let time = date_moment.format("HH:mm")
    return(
        ...
        <div className="mx-20 mt-0 border-2 p-1 mt-2 rounded-md h-max transition-transform" onClick={(id) => setIsOpen(!isOpen)}>
            <div className="content-header-title flex inline-block p-b-2">
                        ...
                        //    checked={props.dataToRender.iSDelete}
                         ...
                         />
                        <span className="dots bg-red-500 w-4 h-4 mx-4 mt-4 rounded-full"></span>
                         ...
                         <span className="bg-black rounded-full h-10 w-10 mt-2 mx-3 text-center text-white text-m p-2">{lname+ " " + fname}</span>
                        ...
                        <span className="float-right">{props.dataToRender.Email_Subject}</span><br/>
                      
            </div>

            {isOpen &&
                <div className="display-expand-data  p-5 border-t-2 border-dashed ">
                <span className=""><b>{props.dataToRender.Sender}</b></span><br/>
                              <span className="text-xs">{props.dataToRender.Date}</span>

                              <p className="text-xs mt-10 font-sans">-------Forwarded Message-------</p><br/>
                              <span className="text-s font-sans">From: <b>{props.dataToRender.Sender} </b></span><span className="inline-block text-s text-sky-500">{"<" + props.dataToRender.Email + ">"}</span><br/>
                              <span className="text-s font-sans">Date: {dateandtime} </span> @ <spa className="inline-block text-s">{time}</spa><br/>
                              <span className="text-s font-sans">Subject: {props.dataToRender.Email_Subject} </span><br/>
                              <span className="text-s font-sans">To: <b>{props.dataToRender.Receiver} </b></span><span className="inline-block text-s text-sky-500">{"<" + props.dataToRender.Receiver_Email + ">"}</span>
...
export default EmailSavedComponent;